  ...
  useEffect(() => {
    const fetchProfileAndRatedRecipes = async () => {
      try {
        ...
        // 4. 5점 평가한 레시피 ID들 가져오기
        const fiveStarRatedRecipeIds = Object.keys(ratings).filter(
          recipeId => ratings[recipeId] === 5,
        );

        // 5. 각 레시피 정보 가져오기
        const recipesData = await Promise.all(
          fiveStarRatedRecipeIds.map(async recipeId => {
            try {
  ...
  return (
    <Container>
      ...
      {ratedRecipes.length > 0 ? (
        ...
        <NoRatingsContainer>
          <NoRatingsText>5점 평가한 레시피가 없습니다.</NoRatingsText>
        </NoRatingsContainer>
      )}
    </Container>
  );
